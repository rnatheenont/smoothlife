...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [updated] = await supabaseRest<AddressRow[]>(`addresses?id=eq.${params.id}&user_id=eq.${uid}`, {
    method: "PATCH",
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await supabaseRest(`addresses?id=eq.${params.id}&user_id=eq.${uid}`, { method: "DELETE", returning: false });
