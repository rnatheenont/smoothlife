...
import { Pencil, MapPin, Receipt, Plus, ChevronRight, ShieldCheck, Package, Heart, Camera, LogOut, UserCheck, X, ScanFace } from "lucide-react";
  ...
  return (
    <div className="rounded-xl2 border border-slate-100 shadow-card p-5">
      ...
      <Link
        ...
        className="flex items-center gap-1.5 text-sm font-semibold text-brand-800 mt-3.5 border-t border-slate-100 pt-3.5"
      >
...
function AddressSummaryCard() {
  ...
  return (
    <div className="rounded-xl2 border border-slate-100 shadow-card p-5">
      ...
      {addr === null && <p className="text-sm text-slate-500 mb-2">ยังไม่มีที่อยู่จัดส่ง</p>}
      <Link
  ...
  return (
    <div className="rounded-xl2 border border-slate-100 shadow-card p-5">
      ...
      {addr ? (
        ...
        <p className="text-sm text-slate-500">ยังไม่มีที่อยู่ใบกำกับภาษี</p>
      )}
      <Link
...
// The latest saved skin scan, so a result doesn't only exist on the page it
// was made on. Opens the full history.
  ...
  return (
    <Link href="/account/change-password" className="rounded-xl2 border border-slate-100 shadow-card p-5 flex items-center gap-3 group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500">ยืนยันตัวตนผ่านอีเมลก่อนตั้งรหัสผ่านใหม่</p>
      </div>
      <ChevronRight size={16} className="text-slate-300 shrink-0 group-hover:translate-x-0.5 transition-transform" />
    </Link>
  );
...
function AccountDashboard() {
  ...
  return (
    <div>
      ...
      <h2 className="text-sm font-bold text-brand-ink mb-3">ผลสแกนผิวของฉัน</h2>
      <div className="mb-8">
        <SkinScanSummaryCard />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <Link href="/account/orders" className="flex flex-col items-start gap-2.5 rounded-xl2 border border-slate-100 bg-white p-4 shadow-card hover:border-brand-teal transition-colors">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-brand-gradient-soft text-brand-800"><Package size={18} /></div>
        ...
        <Link href="/account/wishlist" className="flex flex-col items-start gap-2.5 rounded-xl2 border border-slate-100 bg-white p-4 shadow-card hover:border-brand-teal transition-colors">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-brand-gradient-soft text-brand-800"><Heart size={18} /></div>
        ...
        <Link href="/skin-coach" className="flex flex-col items-start gap-2.5 rounded-xl2 border border-slate-100 bg-white p-4 shadow-card hover:border-brand-teal transition-colors">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-brand-gradient-soft text-brand-800"><Camera size={18} /></div>
