...
import { resizeForUpload, resizeForThumbnail } from "@/lib/image-utils";
import { useGlowChallenge, DayResult } from "@/lib/glow-challenge-context";

export default function CaptureStep({ day }: { day: number }) {
  const { addEntry } = useGlowChallenge();
  ...
  async function handleFile(file: File) {
    ...
    setBusy(true);
    try {
      const [upload, thumb] = await Promise.all([resizeForUpload(file), resizeForThumbnail(file)]);
      ...
      const result = data?.result as DayResult;
      if (!result || !result.acne || !result.darkSpots || !result.wrinkles) {
      ...
      addEntry({ day, capturedAt: new Date().toISOString(), thumbDataUrl: thumb.dataUrl, result });
    } catch {
