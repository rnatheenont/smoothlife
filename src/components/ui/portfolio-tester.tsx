...
// The recipe's own palette was the Shader Builder's default (hot pink, yellow,
// magenta, red) with the post chain that flatters it: +0.5 brightness, 2x
// saturation. Those are the console's brand ramp now, and the same post chain
// would bleach them — #ebf8f4 lifted by 0.5 is white — so contrast, saturation
// and hue are back at neutral and the colours below reach the screen as
// themselves. Brightness keeps a +0.3 lift, which is what turns the deep end of
// the ramp (#05755f) into a tint a table can sit on rather than a block of
// green. Keep these in step with the --color-brand-* tokens in globals.css.
const UNIFORMS = {
  colors: [
    [0.921569, 0.972549, 0.956863], // brand-50   #ebf8f4
    [0.619608, 0.870588, 0.803922], // brand-200  #9edecd
    [0, 0.701961, 0.607843], // brand-teal #00b39b
    [0, 0.682353, 0.937255], // brand-sky  #00aeef
    [0.019608, 0.458824, 0.372549], // brand-800  #05755f
    [0.019608, 0.458824, 0.372549],
    [0.019608, 0.458824, 0.372549],
    [0.019608, 0.458824, 0.372549],
...
/**
 * Five stops from one colour, light end first.
 *
 * A campaign picks one accent and everything on its page is drawn from it, so
 * the background is too — a fixed palette would be the one thing on the page
 * ignoring the setting that governs the rest. Mixing toward white rather than
 * reaching for other hues keeps it a wash of that colour instead of a second
 * colour scheme competing with it.
 */
function rampFrom(accent: string): [number, number, number][] | null {
  const hex = accent.trim()
  if (!/^#[0-9a-fA-F]{6}$/.test(hex)) return null
  const n = parseInt(hex.slice(1), 16)
  const base = [(n >> 16) & 255, (n >> 8) & 255, n & 255]
  const toward = (t: number) =>
    base.map((v) => (v * (1 - t) + 255 * t) / 255) as [number, number, number]
  // Weighted to the saturated end on purpose. shade() returns a weighted mean
  // of the stops and seeds it with the first one, so a ramp spaced evenly from
  // white averages out to a wash — three stops more than half white and the
  // page reads as faintly tinted paper rather than a colour the campaign chose.
  return [toward(0.7), toward(0.46), toward(0.26), toward(0.1), toward(0)]
}
...
export function ShaderBackground({
  ...
  className?: string
  /** #RRGGBB the gradient is mixed from. Falls back to the brand ramp. */
  accent?: string
}) {
  ...
  useEffect(() => {
    ...
    // u_colors is a vec3[8]; the shader only reads the first u_colorCount of
    // them, so the ramp is padded out to eight with its own last stop.
    const ramp = (accent && rampFrom(accent)) || UNIFORMS.colors.slice(0, UNIFORMS.colorCount)
    ...
    // A full-page background that never stops moving is the kind of motion the
    // setting exists for, so a machine that asks for less gets the same
    // gradient held still: one frame, then the loop stops asking for more.
    // The cursor still settles, since that motion is something the reader
    // started.
